...
**위젯 기능:**
- **5h / Daily / Weekly / Monthly** 탭으로 기간별 사용량 확인
- 드래그로 위치 이동
- 접기/펼치기 (─ 버튼)
- 비용 & 토큰 사용량 프로그레스 바
...
**앱 기능:**
- 상단 알림창에 사용량 상시 표시 (5h/일간/주간/월간)
- 프로그레스 바로 예산 사용률 표시
- 알림 펼치면 **기간별 사용량** 한눈에 확인
...
**알림에 표시되는 정보:**
- 현재 비용 / 예산 한도
- 5시간, 오늘, 이번 주, 이번 달 별 비용/토큰/요청수
- 입력/출력 토큰 구분

## 기간별 사용량 (Time Windows)

서버는 모든 API 호출을 로그로 기록하고, 다음 기간별로 자동 집계합니다:

| 기간 | 설명 |
|---|---|
| **5h** | 최근 5시간 사용량 |
| **Daily** | 오늘 자정(UTC) 이후 사용량 |
| **Weekly** | 이번 주 월요일 이후 사용량 |
| **Monthly** | 이번 달 1일 이후 사용량 |

각 기간별로 비용($), 토큰 수, 입력/출력, 요청 횟수를 확인할 수 있습니다.
35일 이상 된 로그는 자동으로 정리됩니다.
